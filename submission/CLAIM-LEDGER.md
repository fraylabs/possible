...
| --- | --- | --- |
| Possible is an outcome compiler for Codex. | `packages/packs/src/types.ts`, `packages/packs/src/compiler.ts`, `skills/possible/SKILL.md`, `ARCHITECTURE.md` | Product description, not a performance result. |
...
| Possible waits for approval before ingredient installation, outcome-state writes, or production execution. | `skills/possible/SKILL.md` under “Recommend one pack” and “Compile after confirmation” | Read-only intake and project inspection may happen before approval; the Possible skill itself was installed earlier. Pack approval authorizes disclosed repo-local work only. |
| Packs define reviewed capabilities, bounded workstreams, outputs, guardrails, and verification. | `packages/packs/src/types.ts`, `packages/packs/src/hardware-launch.ts` | The manifest has no dependency/order or typed definition-of-done field. The compiler supplies a fixed captain sequence; the approved run writes outcome-specific checks into the shared brief. |
...
| The reviewer did not repair implementation. | `apps/web/public/demo/still/CODEX-THREAD.md` at the independent-review lane; `evidence/final-receipt.md` | Captain made the Vite `base: "./"` repair, then review reran. |
...
**Mechanism:** Skills are ingredients. Possible compiles the outcome.
