...
for (const slug of ["hardware-launch", "software-launch", "open-source-release", "playable-web-game", "web-app-operations", "working-web-app", "production-web-release", "marketing-operations", "billion-dollar-saas", "kickstarter-funding", "kickstarter-fulfillment"]) {
  check(catalog.includes(`Slug: \`${slug}\``), `pack reference must include ${slug}`);
}
for (const [slug, lane] of [["hardware-launch", "launch"], ["software-launch", "launch"], ["open-source-release", "release"], ["playable-web-game", "create"], ["web-app-operations", "operate"], ["working-web-app", "create"], ["production-web-release", "release"], ["marketing-operations", "operate"], ["billion-dollar-saas", "create"], ["kickstarter-funding", "launch"], ["kickstarter-fulfillment", "operate"]]) {
...
check((catalog.match(/npx skills@1\.5\.19 add/g) ?? []).length === 28, "pack reference must include all twenty-eight grouped install commands");
