...
for (const slug of ["hardware-launch", "software-launch", "open-source-release", "playable-web-game", "web-app-operations"]) {
  check(catalog.includes(`Slug: \`${slug}\``), `pack reference must include ${slug}`);
}
for (const [slug, lane] of [["hardware-launch", "launch"], ["software-launch", "launch"], ["open-source-release", "release"], ["playable-web-game", "create"], ["web-app-operations", "operate"]]) {
...
check((catalog.match(/npx skills@1\.5\.19 add/g) ?? []).length === 13, "pack reference must include all thirteen grouped install commands");
