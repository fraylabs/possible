...
export function compileRunPrompt(pack: OutcomePack): string {
  ...
  return `Build me a ${pack.name} for the product described below.
...
CAPTAIN WORKFLOW
1. Inspect the workspace and this brief. Do not start production until you write a shared launch-brief.md containing only confirmed product facts, audience, promise, visual direction, constraints, and acceptance checks.
...
3. Create one subagent for each independent workstream below. Give every subagent launch-brief.md, explicit ownership, its named skills, and its own completion verifier. Do not create one subagent per skill.
${workstreams}
4. Continue as captain while the workstreams run: protect the shared facts, resolve interface decisions, and prepare the integration shell. Wait for all workstreams, review their receipts, then integrate them into launch-room/ without erasing unrelated user work.
5. After integration, create a fresh verification subagent. It must invoke $webapp-testing, inspect the actual integrated experience, check every promised artifact, and return evidence—not implementation work.
6. Fix material integration failures, rerun the relevant checks, and finish with a concise launch receipt: created artifacts, verifier commands, passed/failed/skipped checks, known limitations, and every unproven claim.
