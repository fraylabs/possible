...
export const kickstarterFulfillmentPack: OutcomePack = {
  ...
  workstreams: [
    ...
    {
      id: "control-loop",
      name: "Fulfillment control tower and recurring completion report",
      skills: ["analytics", "marketing-loops", "incident-postmortem"],
      owns: ["fulfillment/control/", "fulfillment/receipts/", "fulfillment/schedule/"],
      brief: "Build the canonical milestone and exception state, evidence ingest, decision thresholds, risk review, collision-free dated completion reports, kill switch, and next review date. Run the first cycle from authorized evidence and prepare a scheduling-safe standalone task after the manual cycle passes.",
    },
  ],
  ...
  outputs: [
    ...
    "Recurring fulfillment control loop with first dated completion report and next review",
    "Scheduling-ready task and, only when separately approved, enabled schedule report",
    "Shipment milestone ledger from funded through 95% shipped",
    "Independent fulfillment completion report or explicit blocked/no-go outcome",
  ],
  guardrails: [
    ...
    "Never place personal names, emails, addresses, phone numbers, payment data, full tracking identifiers, or unredacted backer exports in version control, completion reports, screenshots, logs, or benchmark evidence.",
  ...
  verification: [
    ...
    "Run the control loop twice against authorized fixtures, simulate an overlapping run and kill switch, and prove it reads prior state, deduplicates events, carries exceptions forward, and writes collision-free dated completion reports.",
    ...
    "When scheduling is requested, test the standalone prompt manually and record either the separately approved schedule identifier and enabled state or a completion report that records scheduling as no-go.",
  ],
};
